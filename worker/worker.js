/**
 * Cloudflare Worker Enterprise Universal Backend & Real Probe Engine
 * Fully Solves CORS for Subscriptions, Latency Probes, DoH, and Converters
 */
    ...
    try {
      if (pathname === '/' || pathname === '/api') {
        return new Response(JSON.stringify({
          status: 'online',
          service: 'MiSub & CF-Optimizer Real Probe Backend',
          version: '4.0.0'
        }), {
      ...
      if (pathname === '/api/probe') {
        ...
        if (!ip) return new Response(JSON.stringify({ error: 'ip is required' }), { status: 400, headers: CORS_HEADERS });
      ...
      // 6. Direct Client Subscription Provider Endpoint (/sub)
      if (pathname === '/sub') {
